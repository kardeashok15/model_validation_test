...
csvFile = os.path.join(
    BASE_DIR, "modelvalidation\\insurance_claims.csv")
...
count_target = df['fraud_reported12'].value_counts()
print(count_target)
...
sns_plot.savefig(os.path.join(
    BASE_DIR, 'static\\media\\'+user_name+'pairplot.png'))
...
plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'stackedbar.png'))
plt.close()
i = 5
j = 12
var1 = dict_cat[i]
...
plt.legend()
plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'bar.png'))
plt.close()
i = 5
j = 12
var1 = dict_cat[i]
...
plt.legend()
plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'stackedbar2.png'))
plt.close()
...
plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'joyplot.png'))
plt.close()
...
fig.write_image(os.path.join(
    BASE_DIR, 'static\\media\\'+user_name+'boxplot3d.png'))
i = 1
j = 3
k = 9
l = 13
...
fig.write_image(os.path.join(
    BASE_DIR, 'static\\media\\'+user_name+'scatterd3d.png'))
...
def evaluate_model(val_pred, val_probs, train_pred, train_probs):
    ...
    plt.savefig(os.path.join(
        BASE_DIR, 'static\media\\'+user_name+'roccurve.png'))
    plt.close()
...
plt.savefig(os.path.join(
    BASE_DIR, "static\\media\\" + user_name+'confmatrixTrain.png'))
plt.close()
...
plt.xlabel('Predicted label')
plt.savefig(os.path.join(
    BASE_DIR, "static\\media\\" + user_name+'confmatrixValidationpython .png'))
plt.close()
