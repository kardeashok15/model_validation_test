...
def table(request):
    try:
        ...
        return render(request, 'showdata.html', {'desc': arrdescData, 'dataTypes': gridDttypes, 'df': result})
...
def viewDataType(request):
    try:
        ...
        pdf.output(os.path.join(
            BASE_DIR, "static\\media\\DatatypeCount.pdf"))
        del df
        return render(request, 'viewDataType.html',  {'pdfFile': '\static\media\DatatypeCount.pdf', 'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def update_missingval(request):
    ...
    df.to_csv(savefile_withoutnull, index=False)
    processing = os.path.join(BASE_DIR, 'static\media\processing.csv')
    df_old_proc = pd.read_csv(processing)
...
def dataCleaning(request):
    try:
        ...
        return render(request, 'dataCleaning.html', {'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def deleteColumns(request):
    ...
    x1.to_csv(savefile_x, index=False)
    ...
    # y.to_csv(savefile_y, index=False)
...
def showCatColFreq(request):
    try:
        ...
        return render(request, 'showFreqData.html', {'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def dropfeatures(request):
    try:
        ...
        return render(request, 'dropfeatures.html', {'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def showSNSChart(request):
    try:
        ...
        sns_plot.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'output.png'))
        return render(request, 'showSNSChart.html', {'graphpath': '\\static\\media\\'+user_name+'output.png'})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def showPieChart(request):
    try:
        ...
        fig.write_image(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputPieChart.png'))
        context = {'graphpath': '\\static\\media\\'+user_name+'outputPieChart.png',
                   'plot_div': Markup(plot_div), 'hideddls': 'none', 'hideUnvar': 'block'}
...
def showDistPlot(request):
    ...
    fig.savefig(os.path.join(BASE_DIR, 'static\\media\\' +
                user_name+'outputDistPlot.png'))

    context = {'graphpath': '\\static\\media\\'+user_name+'outputDistPlot.png'}
...
def showBoxPlot(request):
    try:
        ...
        context = {'graphpath': '\\static\\media\\' +
                   user_name+'outputBoxPlot.png'}
...
def showCatCountPlot(request):
    try:
        ...
        fig.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputCatCntPlot.png'))

        context = {'graphpath': '\\static\\media\\' +
                   user_name+'outputCatCntPlot.png'}
...
def plotinsured_occupations(request):
    try:
        ...
        plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'plotinsured_occupations.png'))
        plt.close()
        context = {'graphpath': '\\static\\media\\'+user_name+'plotinsured_occupations.png',
...
def plotinsoccuvsincstate(request):
    try:
        ...
        color = plt.cm.Blues(np.linspace(0, 1, 5))
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'plotinsoccuvsincstate.png'))
        plt.close()
        del df
        del x_keep
        saveChartViewd('Bar chart', var1, var2, 'plotinsoccuvsincstate.png')
        if os.path.exists(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'plotinsoccuvsincstate.png')):
            pdf = FPDF()
            pdf.add_page()
            pdf = exportgraphImgPdf(pdf, os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'plotinsoccuvsincstate.png'), " Bar chart "+var1+" vs "+var2)
            pdf.output(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'Bar chart.pdf'))

        context = {'chartType': 'Bar chart', 'pdfFile': '\\static\\media\\'+user_name+'Bar chart.pdf', 'graphpath': '\\static\\media\\'+user_name+'plotinsoccuvsincstate.png',
...
def plotinsoccuvsincstatestacked(request):
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'plotinsoccuvsincstatestacked.png'))
        plt.close()
        del df
        saveChartViewd('Stacked Bar chart', var1, var2,
                       'plotinsoccuvsincstatestacked.png')
        ...
        if os.path.exists(os.path.join(
            ...
            pdf.add_page()
            pdf = exportgraphImgPdf(pdf, os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'plotinsoccuvsincstatestacked.png'), " Stacked Bar chart "+var1+" vs "+var2)
            pdf.output(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'Stacked Bar chart.pdf'))

        context = {'chartType': 'Stacked Bar chart', 'pdfFile': '\\static\\media\\'+user_name+'Stacked Bar chart.pdf', 'graphpath': '\\static\\media\\'+user_name+'plotinsoccuvsincstatestacked.png', 'ddlvar1': cat_cols,
...
def var_dist_by_fraud_old(request):
    try:
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'distbyfraud.png'))
        plt.close()
        context = {'graphpath':  '\\static\\media\\'+user_name+'distbyfraud.png',
...
def pairwise_correlation(request):
    try:
        ...
        fig1.write_image(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'pairwise_correlation_fig1.png'))

        fig2.write_image(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'pairwise_correlation_fig2.png'))
...
def vardistbyfraud(request):
    try:
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'distbyfraud2.png'))
        saveChartViewd('Distribution', var_num, var_cat, 'distbyfraud2.png')
        del df
        if os.path.exists(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'distbyfraud2.png')):
            pdf = FPDF()
            pdf.add_page()
            pdf = exportgraphImgPdf(pdf, os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'distbyfraud2.png'), " Distribution "+var_num+" vs "+var_cat)
            pdf.output(os.path.join(
                BASE_DIR, "static\\media\\"+user_name+"Distribution.pdf"))

        context = {'chartType': 'Distribution', 'pdfFile': '\\static\\media\\'+user_name+'Distribution.pdf', 'graphpath': '\\static\\media\\'+user_name+'distbyfraud2.png',
...
def stripplot(request):
    try:
        ...
        pltstrip.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputstripplot.png'))
        pltstrip.close()
        saveChartViewd('Strip Plot', var_num, var_cat, 'outputstripplot.png')
        del df
        if os.path.exists(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'outputstripplot.png')):
            pdf = FPDF()
            pdf.add_page()
            pdf = exportgraphImgPdf(pdf, os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'outputstripplot.png'), " Strip Plot "+var_num+" vs "+var_cat)
            pdf.output(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'Strip Plot.pdf'))

        context = {'chartType': 'Strip Plot', 'pdfFile': '\\static\\media\\'+user_name+'Strip Plot.pdf', 'graphpath': '\\static\\media\\'+user_name+'outputstripplot.png',
...
def totalclaim_boxplot(request):
    try:
        ...
        pltbox.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputclaimboxplot.png'))
        pltbox.close()
        saveChartViewd('Box Plot', var_num, var_cat, 'outputclaimboxplot.png')
        del df
        if os.path.exists(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'outputclaimboxplot.png')):
            pdf = FPDF()
            pdf.add_page()
            pdf = exportgraphImgPdf(pdf, os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'outputclaimboxplot.png'), " Box Plot "+var_num+" vs "+var_cat)
            pdf.output(os.path.join(
                BASE_DIR, 'static\\media\\'+user_name+'Box Plot.pdf'))

        context = {'chartType': 'Box Plot', 'pdfFile': '\\static\\media\\'+user_name+'Box Plot.pdf', 'graphpath': '\\static\\media\\'+user_name+'outputclaimboxplot.png',
                   'ddlvar1': num_cols, 'ddlvar2': cat_cols, 'var1': var_num, 'var2': var_cat, 'hideddl2': '', 'postAct': totalclaim_boxplot}
        return render(request, 'showPlot.html', context)
    except Exception as e:
        print("Error is ", e)
...
def scattred3d(request):
    try:
        ...
        plot_div = plot(fig, include_plotlyjs=False, output_type='div')
        fig.write_image(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputscattred3d.png'))
        context = {'graphpath': '\\static\\media\\'+user_name+'outputscattred3d.png',
                   'plot_div': Markup(plot_div), 'var1': var1,
                   'var2': var2, 'var3': var3, 'ddlvar1': num_cols, 'ddlvar2': num_cols, 'ddlvar3': cat_cols, 'hideUnvar': 'none', 'displayddl3': '', 'postAct': scattred3d}
        return render(request, 'show3dplot.html', context)
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def bubblePlot3d(request):
    try:
        ...
        context = {'graphpath': '\\static\\media\\'+user_name+'outputscattred3d.png',
                   'plot_div': Markup(plot_div), 'var1': var1,
                   'var2': var2, 'ddlvar1': num_cols, 'ddlvar2': num_cols, 'displayddl3': 'none', 'hideUnvar': 'none', 'postAct': bubblePlot3d}
        return render(request, 'show3dplot.html', context)
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def showcorrelation(request):
    try:
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputcorrelation.png'), dpi=400)
        # (result)

        saveChartViewd("Heatmap", "", "", 'outputcorrelation.png')
        # if os.path.exists(os.path.join(
        #         BASE_DIR, 'static\media\outputcorrelation.png')):
        pdf = FPDF()
        pdf.add_page()
        pdf = exportgraphImgPdf(pdf, os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputcorrelation.png'),  "Correlation on independent variables-Heat map", "")
        pdf.output(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'Heatmap.pdf'))

        return render(request, 'showCorrelation1.html', {'pdfFile': '\\static\\media\\'+user_name+'Heatmap.pdf', 'df': result, 'ColNames': dfcorr.columns, 'graphpath': '\\static\\media\\'+user_name+'outputcorrelation.png'})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def showSNScorrelation(request):
    try:
        ...
        fig.savefig(os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'outputcorrelation.png'), dpi=400)
        return render(request, 'showSNSHeatMap1.html', {'graphpath': 'static\\media\\'+user_name+'outputcorrelation.png'})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def dist_numevari_catvar(request):
    try:
        ...
        result = json.loads(result)
        return render(request, 'showdistnumcat.html', {'df': result, 'cat_var': cat_var, 'num_var': num_var, 'colNames': dist_num_cat.columns, 'numCols': num_cols, 'catCols': cat_cols, 'divHeader': 'distribution of ' + num_var + ' at ' + cat_var})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def detect_outliers(request):
    try:
        ...
        #     return anomalies
            # print(anomalies)
            objlstOutlieranomalies.arr_anomalies = anomalies
...
def updateData(request):
    try:
        ...
        return render(request, 'updateData.html', {'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def showTargetColFreq(request):
    try:
        ...
        if os.path.exists(csvfile):
            ...
            gridDttypes.append(objlstColFreq)
        return render(request, 'resamplingData.html', {'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
...
def getSubTitleCS(request):
    ...
    data = {}
    report_file_path = os.path.join(BASE_DIR, 'static\media\\')
    report_file_name = "temp_CS_"+user_name
...
def getSubTitle(request):
    ...
    exeSumm = ['Model Purpose and Use',
               ...
               'Validation Findings']
    if(title == "1"):
        data = {'subTtl': exeSumm, 'titleIdx': titleIdx}
    elif(title == "-1"):
        data = {'subTtl': [], 'titleIdx': titleIdx}
    elif(title == "2"):
        data = {'subTtl': ['Development Overview', 'Development Documentation',
                           'Input and Data Integrity', 'Conceptual Soundness'], 'titleIdx': titleIdx}
    elif(title == "3"):
        data = {'subTtl': ['Model Diagnostic Testing', 'Outcome Analysis / Back-testing',
                           'Benchmarking', 'Sensitivity, Stability, and Robustness'], 'titleIdx': titleIdx}
    elif(title == "4"):
        data = {'subTtl': [
            'Production Platform, Data, and Code', 'Implementation Plan'], 'titleIdx': titleIdx}
    elif(title == "5"):
        data = {'subTtl': ['Performance and Risk Monitoring', 'Change Management',
                           'Tuning and Calibration', 'Model Reference Tables'], 'titleIdx': titleIdx}
    return JsonResponse(data)
...
def getSubSubTitle(request):
    ...
    if os.path.exists(cnfrmsrcFiles):
        ...
        titleIdx = str(titleIdx)
    if(title == "2.4"):
        data = {'subsubTtl': ['Methodology', 'Suitability', 'Variable Selection and Segmentation',
                              'Development Platform and Code', 'Assumptions', 'Limitations'], 'titleIdx': titleIdx}
    elif(title == "2.3"):
        data = {'subsubTtl': [
            'Input Data Source', 'Data Transformation, Cleaning', 'Final Model Development Dataset'], 'titleIdx': titleIdx}
    else:
        data = {'subsubTtl': [], 'titleIdx': titleIdx}
...
def getSubSubTitleIdx(request):
    ...
    if os.path.exists(cnfrmsrcFiles):
        ...
        if(len(dffilter) > 0):
            titleIdx = dffilter["subsubtitle"].max()
            titleIdx = str(titleIdx).split(".")[1]
            titleIdx = int(titleIdx)-1
        else:
            dffilter = df.query("title == '" + titleTxt +
        ...
        if (df["chartType"] == chartType).any():
            ...
            if (df["chartType"] == chartType).any():
                ...
                for index, row in dffilter.iterrows():
                    if os.path.exists(os.path.join(
                            BASE_DIR, 'static\\media\\'+user_name+row["imageName"])):
                        pdf.add_page()
                        if(row["chartType"] == "Heatmap"):
                            pdf = exportgraphImgPdf(pdf, os.path.join(
                                BASE_DIR, 'static\\media\\'+user_name+row["imageName"]),  "Correlation on independent variables-Heat map", comments)
                        else:
                            pdf = exportgraphImgPdf(pdf, os.path.join(
                                BASE_DIR, 'static\\media\\'+user_name+row["imageName"]), row["chartType"]+" "+row["xaxisval"]+" vs "+row["yaxisval"], comments)
                        pdf.output(os.path.join(
                            BASE_DIR, 'static\\media\\'+user_name+chartType+'.pdf'))
                del dffilter
...
def saveChartImage(request):
    ...
    UserCommentsFiles = file_path + "_ChartViewd.csv"
    directory = os.path.join(BASE_DIR, 'static\\media\\'+user_name+'Chartimgs')
    ...
    if os.path.exists(UserChartFile):
        ...
        if (df2["chartType"] == chartType).any():
            ...
            destination = 'static\\media\\'+user_name+'Chartimgs\\'+chartImg+'.png'
            df2.loc[df.chartType ==
            ...
            dfmerged.to_csv(UserChartFile, index=False)
            del dfmerged
            del dffilter
        if (df["chartType"] == chartType).any():
            dffilter = df.query("chartType== '"+chartType+"'")
            for index, row in dffilter.iterrows():
                if os.path.exists(os.path.join(
                    ...
                    destination = os.path.join(
                        BASE_DIR, 'static\\media\\'+user_name+'Chartimgs\\'+chartImg+'.png')
        ...
        dfnew = pd.DataFrame(
            data, columns=['chartType', 'chartImg', 'destination', 'comments'])
        dfnew.to_csv(UserChartFile, index=False)
        del dfnew
        ...
        if (df["chartType"] == chartType).any():
            ...
            for index, row in dffilter.iterrows():
                if os.path.exists(os.path.join(
                    ...
                    print("File copied successfully.")
            del dffilter
    if os.path.exists(os.path.join(BASE_DIR, 'static\\media\\'+user_name+chartType+'.pdf')):
        ...
        # Destination path
        destination = os.path.join(
            BASE_DIR, 'static\\media\\'+user_name+'Chartimgs\\'+chartImg+'.pdf')
        shutil.copyfile(source, destination)
...
def updateContacts(request):
    ...
    if os.path.exists(contactFile):
        ...
        if len(dffilter) > 0:
            ...
            print('df_old ', df_old)
            df_old.to_csv(contactFile, index=False)
            del df_old, dffilter
        else:
            ...
            df = pd.concat([df_old, df_new], axis=0)
            df.to_csv(contactFile, index=False)
