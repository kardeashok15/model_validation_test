...
def table(request):
    try: 
        ...
        if(statusReq == "Not done").any():
            return render(request, 'modelValReq.html')
        ...
        if request.method == 'POST' and request.FILES['myfile']:
            ...
            fs.save(file_path + file_name + "_EDA.csv", myfile) 
            processing = os.path.join(BASE_DIR, 'static/reportTemplates/processing.csv')
            df_old_proc = pd.read_csv(processing)
        ...
        if os.path.exists(savefile_name):
            ...
            if os.path.exists(savefile_name+".hdf5"):
                 os.remove(savefile_name+".hdf5")
            if os.path.exists(savefile_name+".yaml"):
                 os.remove(savefile_name+".yaml")
            dfvx = vx.from_csv(savefile_name, convert=True) 
            if not os.path.exists(file_path + file_name + "_catcols.csv"):
                cat_cols_temp = [c for i, c in enumerate(
                dfvx.columns) if dfvx.dtypes[i] =='string']

                cat_cols=[]
                for x in cat_cols_temp:
                    if len(dfvx[x].value_counts())<25:
                        cat_cols.append(x)

                dfcat = pd.DataFrame(
                cat_cols, columns=['colName'])
                dfcat.to_csv(file_path + file_name + "_catcols.csv", index=False)
                del dfcat
            if not os.path.exists(file_path + file_name + "_numcols.csv"):
                ...
                dfnum = pd.DataFrame(
                num_cols, columns=['colName'])
                dfnum.to_csv(file_path + file_name + "_numcols.csv", index=False)
                del dfnum

            gen=dfvx.to_pandas_df(chunk_size=5_000_000)
            del dfvx
...
def selCols(request):
    try:
        ...
        if(not os.path.exists(savefile_name)):
            return render(request, 'processNotdone.html')
        dfvx = vx.from_csv(savefile_name, convert=True) 
        gen=dfvx.to_pandas_df(chunk_size=10)
        for i1, i2, chunk in gen:
            df=chunk 
            break
        del dfvx,chunk
...
def viewNumData(request):
    try:
        ...
        if(not os.path.exists(savefile_name)):
            return render(request, 'processNotdone.html')
        # df = pd.read_csv(savefile_name, na_values='?')
        df=vx.from_csv(savefile_name, convert=True) 
        num_cols = [c for i, c in enumerate(
...
def dataCleaning(request):
    try:
        ...
        if(not os.path.exists(savefile_withoutnull)):
            return render(request, 'processNotdone.html') 
        dfvx = vx.from_csv(savefile_withoutnull, convert=True) 
        gen=dfvx.to_pandas_df(chunk_size=10)
        for i1, i2, chunk in gen:
            df=chunk
            break
...
def deleteColumns(request):
    ...
    t1=time.time()
    savefile_withoutnull = file_path + file_name + ".csv"
    dfvx =vx.from_csv(savefile_withoutnull, convert=True)  #pd.read_csv(savefile_withoutnull, na_values='?')
    ...
    print('time taken to save file ',time.time()-t1)
...
def showSNSChart(request):
    try: 
        ...
        savefile_x_keep = file_path + file_name + "_EDA.csv"
        ...
        print('time taken by pd pairplot is ',time.time()-t1)
...
def showPieChart(request):
    try:
        t1=time.time()
        savefile_withoutnull = file_path + file_name + "_EDA.csv"
        if(not os.path.exists(savefile_withoutnull)):
            return render(request, 'processNotdone.html')
        targetVarFile = file_path + file_name + "_targetVar.txt"
        file1 = open(targetVarFile, "r")  # write mode
        ...
        print('time taken by pd pie chart is ',time.time()-t1)
...
def showDistPlot(request):
    ...
    print('time taken by pd dist chart is ',time.time()-t1)
...
def showBoxPlot(request):
    try:
        ...
        print('time taken by pd box chart is ',time.time()-t1)
...
def plotinsoccuvsincstate(request):
    try:
        ...
        print('varr1 ', var1,', var2 ', var2)
        cat_bar = pd.crosstab(df[var1], df[var2])
        print('chart data ',cat_bar)
        ...
        print('time taken by vaex mod bar chart is ', time.time()-t1)
...
def plotinsoccuvsincstatestacked(request):
    try:
        ...
        print('time taken by pd stack bar chart is ', time.time()-t1)
...
def var_dist_by_fraud_old(request):
    try:
        ...
        plt.close()
        print('time taken by pd dist plot is ',time.time()-t1)
...
def vardistbyfraud(request):
    try:
        ...
        print('time taken by multivar dist is ',time.time()-t1)
...
def stripplot(request):
    try:
        ...
        print('time taken by vaex strip plot ', time.time()-t1)
...
def totalclaim_boxplot(request):
    try:
        ...
        print('time taken by viex box plot is ',time.time()-t1)
...
def vehicle_claim(request):
    try:
        ...
        print('time taken by pd boxchart 3d is ',time.time()-t1)
...
def showcorrelation(request):
    try:
        ...
        if(not os.path.exists(savefile_x)):
            return render(request, 'processNotdone.html')
        # x = pd.read_csv(savefile_x)
        dfvx = vx.from_csv(savefile_x, convert=True) 
        x=dfvx.to_pandas_df()
        del dfvx
...
def dist_numevari_catvar(request):
    try:
        ...
        if(not os.path.exists(savefile_withoutnull)):
            return render(request, 'processNotdone.html')
        print('before iteration ',time.time())
        # df = pd.read_csv(savefile_withoutnull, na_values='?')
        dfvx = vx.from_csv(savefile_withoutnull, convert=True) 
        df=dfvx.to_pandas_df()
        del dfvx
        ...
        if(len(cat_cols)<1):
            noData="Categorical variables not available to run this utility."
            return render(request, 'showdistnumcat.html', {'noData':noData})
        result=[]
        cat_var=[]
        if(len(cat_cols)>0):
            num_cols = [c for i, c in enumerate(
...
def getTargetColVals(request):
    # savefile_withoutnull = file_path + file_name + "_withoutnull.csv"
    savefile_withoutnull = file_path + file_name + ".csv"
    df = vx.from_csv(savefile_withoutnull, convert=True)   #pd.read_csv(savefile_withoutnull, na_values='?')
...
def updateData(request):
    try:
        ...
        if os.path.exists(savefile_withoutnull):
            # df = pd.read_csv(savefile_withoutnull, na_values='?')
            dfvx = vx.from_csv(savefile_withoutnull, convert=True) 
            gen=dfvx.to_pandas_df(chunk_size=1000000)
            del dfvx
            for i1, i2, chunk in gen:
                df=chunk
                break   
        ...
        if(len(dffilter) > 0):
            ...
            for index, row in df_sorted.iterrows():
                if row["section"] == "Comment":                    
                    if(str(row["subtitleIdx"])=="0.0" or str(row["subtitleIdx"])=="0"):
...
def impCtrl(request):
    try:
        ...
        print('DocumentationData is ',DocumentationData)
        ...
        print(e)
        print('stcktrace is ',traceback.print_exc())
        return render(request, 'error.html')
...
def WIP(request):
    ...
    #     <center>
    #     <img src="static\\media\\user1KNN_NT_roc1.png" width="600" height="200">
    #     </center>
    #     <h3>Sample List</h3>
    #     <ul><li>option 1</li>
    ...
    # pdf.write_html(html)
    # pdf.output(os.path.join(
    #     BASE_DIR, "static\\media\\htmltest.pdf"), 'F')
    # return render(request, 'test.html')
    return render(request, 'comingsoon.html')

def getValidCatCols():
    savefile_withoutnull = file_path + file_name + "_catcols.csv" 
    df = pd.read_csv(savefile_withoutnull)
...
def getValidNumCols():
    savefile_withoutnull = file_path + file_name + "_numcols.csv" 
    df = pd.read_csv(savefile_withoutnull)
...
def sampleEDAData(request):
    try:
        ...
        pdf.output(os.path.join(
            BASE_DIR, "static/media/DatatypeCount.pdf"))
        del df
        return render(request, 'sampleEDAData.html',  {'page':'sampleEDAData', 'dataTypes': gridDttypes})
    except Exception as e:
        print(e)
        print('stacktrace iis ', traceback.print_exc())
        return render(request, 'error.html')
...
def sampleDateRange(request):
    try:
        from datetime import datetime
        selCol = request.GET['column']
        savefile_name = file_path + file_name + ".csv"
        if(not os.path.exists(savefile_name)):
            return render(request, 'processNotdone.html')
        dfvx = vx.from_csv(savefile_name, convert=True) 
        ...
        rowCnt=""
        try:
            df[selCol] = pd.to_datetime(df[selCol], format='%Y%m%d').dt.strftime('%Y/%m/%d')
            
            print('dates are ',df[selCol].min(),df[selCol].max())  
            print(datetime.strptime(str(df[selCol].min()), "%Y/%m/%d").strftime("%m/%d/%Y"))
            mindt=df[selCol].min()
            maxdt=df[selCol].max()

          
            pdnew=df[(df[selCol] > ' 2021/10/01') & (df[selCol] < '2022/03/01')]
            rowCnt=(len(df))
        except Exception as e:
            del df
            print(e)
        ...
        del df
        return JsonResponse({'is_valid':True,'minDate':mindt, 'maxDate':maxdt,'rowCnt':rowCnt })
    except Exception as e:
        print(e)
        print('stacktrace iis ', traceback.print_exc()) 

def CheckDagta(request):
    try:
        from datetime import datetime
        selCol = request.GET['column']
        fromDt = request.GET['fromDt']
        toDt = request.GET['toDt']
        savefile_name = file_path + file_name + ".csv"
        if(not os.path.exists(savefile_name)):
            return render(request, 'processNotdone.html')
        dfvx = vx.from_csv(savefile_name, convert=True) 
        ...
        try:
            ...
            if len(fromDt)>0 and  len(toDt)>0: 
                fromDt =datetime.strptime(fromDt, "%m/%d/%Y").strftime("%Y/%m/%d")
                toDt =datetime.strptime(toDt, "%m/%d/%Y").strftime("%Y/%m/%d")
                pdnew=df[(df[selCol] > fromDt) & (df[selCol] <toDt)]
            elif len(fromDt)>0: 
                fromDt =datetime.strptime(fromDt, "%m/%d/%Y").strftime("%Y/%m/%d")
                pdnew=df[(df[selCol] > fromDt)]
            elif len(toDt)>0: 
                toDt =datetime.strptime(toDt, "%m/%d/%Y").strftime("%Y/%m/%d")
                pdnew=df[(df[selCol] <toDt)] 
            rowCnt=(len(pdnew))
        except Exception as e:
            del df
            print(e)
...
def saveEDASampleData(request):
    try:
        from datetime import datetime
        selCol = request.GET['column']
        ColLst = request.GET['columns']
        fromDt = request.GET['fromDt']
        toDt = request.GET['toDt']
        sampleType = request.GET['sampleType']
        numRows = request.GET['numRows']
        savefile_name = file_path + file_name + ".csv" 
        json_dictionary = json.loads(ColLst)
        selcolLst = []
        for colval in json_dictionary:
            for attribute, value in colval.items():
                if(attribute == 'column'):
                    selcolLst.append(value)   
        if(not os.path.exists(savefile_name)):
            return render(request, 'processNotdone.html')
        dfvx = vx.from_csv(savefile_name, convert=True) 
        df=dfvx.to_pandas_df()
        ...
        maxdt=""
        rowCnt=""
        # part_data1=df.sample(frac=0.1, random_state=123)
        # part_data2=df.sample(n=100000, random_state=123)
        try:
            ...
            if len(fromDt)>0 and  len(toDt)>0:
                ...
                df[selCol] = pd.to_datetime(df[selCol], format='%Y%m%d').dt.strftime('%Y/%m/%d') 
                pdnew=df[(df[selCol] <toDt)]
            pdnew = pdnew[selcolLst] 
            if len(numRows)>0:
                if(sampleType=="Frac"):
                    pdnew=pdnew.sample(frac=(float(numRows)*0.01) ,random_state=123)
                else:
                    pdnew=pdnew.sample(n=int(numRows),random_state=123)
            rowCnt=(len(pdnew))
            pdnew.to_csv(file_path + file_name + "_EDA.csv", index=False, encoding='utf-8') 
            cat_cols_temp = [c for i, c in enumerate(
            pdnew.columns) if pdnew.dtypes[i] in [np.object]]

            cat_cols=[]
            for x in cat_cols_temp:
                if len(pdnew[x].value_counts())<25:
                    cat_cols.append(x)

           
            
            num_cols_temp = [c for i, c in enumerate(
            pdnew.columns) if pdnew.dtypes[i] not in [np.object]]
            ...
            for x in num_cols_temp:
                if len(pdnew[x].value_counts())<25:
                    num_cols.append(x)
            if len(num_cols)<1 :
                return JsonResponse({'is_valid':True,'message':'Valid numeric variable does not exists in selected dataset.' })

            if len(cat_cols)<1:
                return JsonResponse({'is_valid':True,'message':'Valid categorical variable does not exists in selected dataset. ' })
            ...
            return JsonResponse({'is_valid':False})
        del df
        return JsonResponse({'is_valid':True,'message':'EDA sample dataset created.' })
    except Exception as e:
        print(e)
        print('stacktrace iis ', traceback.print_exc()) 
